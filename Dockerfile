## HEI Work — Dockerfile multi-stage
## Web (Next.js standalone) + Worker (compilado a CommonJS) en un solo container
...
# El worker necesita 'pg' fuera del bundle de Next, asi que reusamos node_modules de prod
...
# Worker en background, server.js (Next standalone) en foreground.
# Si el worker muere, el server sigue. Coolify reinicia el container si server.js muere.
CMD ["sh", "-c", "node dist/worker/index.js & exec node server.js"]
