...
const PRIORITY_STYLE: Record<string, string> = {
  ...
  low: "bg-neutral-500/20 text-[var(--fg-secondary)] border-neutral-500/40",
};
...
export default function TaskCard({ task, isDragging, onDragStart, onDragEnd }: Props) {
  return (
    <Link
      ...
      onDragEnd={onDragEnd}
      className={`block rounded-md border border-[var(--border-base)] bg-[var(--bg-elevated)] p-3 text-left transition hover:border-[var(--border-strong)] ${
    ...
    >
      <div className="flex items-start gap-2">
        ...
        />
        <p className="flex-1 text-sm text-[var(--fg-primary)] leading-snug">{task.title}</p>
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        {task.comment_count > 0 && (
          <span className="text-[10px] text-[var(--fg-muted)]">{task.comment_count}c</span>
        )}
